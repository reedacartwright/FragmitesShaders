...
$input v_clipPosition, v_color0, v_dithering, v_fog, v_lightmapUV, v_texcoord0, v_worldPos, v_worldPosition, v_position
...
void RenderChunkApplyUseless(FragmentInput fragInput, StandardSurfaceInput surfaceInput, inout StandardSurfaceOutput surfaceOutput, inout FragmentOutput fragOutput) {
    int redstone_overlay_type = 0;
#if defined(ENABLE_REDSTONE_OVERLAY) && defined(ALPHA_TEST_PASS)
    if(length(fragInput.worldPos) < 64.0) {
        redstone_overlay_type = redstone_overlay(fragInput.position, surfaceInput.Color);
