...
$output v_lightmapUV, v_texcoord0, v_worldPos, v_worldPosition, v_position
// clang-format on
...
float calculateFogIntensityFadedVanilla(float cameraDepth, float maxDistance,
...
void transformAsBillboardVertex(inout StandardVertexInput stdInput,
...
float RenderChunkVert(StandardVertexInput stdInput,
                      inout VertexOutput vertOutput) {
  ...
  float fogIntensity = calculateFogIntensityFadedVanilla(
      cameraDepth, MixedFogAndDistanceControl.z, MixedFogAndDistanceControl.x,
      MixedFogAndDistanceControl.y, RenderChunkFogAlpha.x);
  vertOutput.fog = vec4(FogColor.rgb, fogIntensity);
#endif
  vertOutput.clipPosition = jitterVertexPosition(stdInput.worldPos);
  return cameraDepth;
}
void RenderChunkVertTransparent(StandardVertexInput stdInput,
                                inout VertexOutput vertOutput) {
...
void StandardTemplate_VertSharedTransform(inout StandardVertexInput stdInput,
                                          inout VertexOutput vertOutput) {
  VertexInput vertInput = stdInput.vertInput;
#ifdef INSTANCING__ON
  mat4 model;
  model[0] = vec4(vertInput.instanceData0.x, vertInput.instanceData1.x,
                  vertInput.instanceData2.x, 0);
  model[1] = vec4(vertInput.instanceData0.y, vertInput.instanceData1.y,
                  vertInput.instanceData2.y, 0);
  model[2] = vec4(vertInput.instanceData0.z, vertInput.instanceData1.z,
                  vertInput.instanceData2.z, 0);
  model[3] = vec4(vertInput.instanceData0.w, vertInput.instanceData1.w,
                  vertInput.instanceData2.w, 1);
...
void StandardTemplate_VertexPreprocessIdentity(VertexInput vertInput,
                                               inout VertexOutput vertOutput) {}

void StandardTemplate_InvokeVertexPreprocessFunction(
    inout VertexInput vertInput, inout VertexOutput vertOutput);
void StandardTemplate_InvokeVertexOverrideFunction(
    StandardVertexInput vertInput, inout VertexOutput vertOutput);
void StandardTemplate_InvokeLightingVertexFunction(
    VertexInput vertInput, inout VertexOutput vertOutput, vec3 worldPosition);

void computeLighting_RenderChunk_Vertex(VertexInput vInput,
  ...
  uvec2 dither = vInput.lightmapUV & uvec2(256u);
  vOutput.ditheringAndMaskTinting = vec2(notEqual(dither, uvec2(0u)));
}
void StandardTemplate_VertShared(VertexInput vertInput,
                                 inout VertexOutput vertOutput) {
  ...
  StandardTemplate_InvokeLightingVertexFunction(vertInput, vertOutput,
                                                stdInput.worldPos);
}
void StandardTemplate_InvokeVertexPreprocessFunction(
    inout VertexInput vertInput, inout VertexOutput vertOutput) {
...
void StandardTemplate_Opaque_Vert(VertexInput vertInput,
                                  inout VertexOutput vertOutput) {
...
void main() {
  ...
  vertexOutput.ditheringAndMaskTinting = vec2(0.0);
  ...
  MixedFogAndDistanceControl =
      mix(FogAndDistanceControl, vec4(0.99, 1.0, 100000.0, 100000.0),
          bvec4(MeshContext.x > 0.5));

  StandardTemplate_Opaque_Vert(vertexInput, vertexOutput);
  v_clipPosition = vertexOutput.clipPosition;
  v_color0 = vertexOutput.color0;
  v_ditheringAndMaskTinting = vertexOutput.ditheringAndMaskTinting;
